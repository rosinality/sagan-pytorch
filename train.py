...
def train(args, n_class, generator, discriminator):
    ...
    for i in pbar:
        ...
        fake_image = generator(
            torch.randn(b_size, args.code).to(device),
            label.to(device))
        ...
        if (i + 1) % 1 == 0:
            ...
            input_class = torch.multinomial(torch.ones(n_class),
                                            args.batch,
                                            replacement=True).to(device)
            fake_image = generator(
                torch.randn(args.batch, args.code).to(device),
                input_class)
        ...
        if (i + 1) % 10000 == 0:
            ...
            torch.save(generator.state_dict(),
                       f'checkpoint/generator_{no}.pt')
            torch.save(discriminator.state_dict(),
                       f'checkpoint/discriminator_{no}.pt')
            torch.save(g_optimizer.state_dict(),
                       f'checkpoint/gen_optimizer_{no}.pt')
            torch.save(d_optimizer.state_dict(),
                       f'checkpoint/dis_optimizer_{no}.pt')

        pbar.set_description((f'{i + 1}; G: {gen_loss_val:.5f};'
                              f' D: {disc_loss_val:.5f}'))
